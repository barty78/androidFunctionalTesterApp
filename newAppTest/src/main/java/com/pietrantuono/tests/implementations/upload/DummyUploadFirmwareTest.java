...
public class DummyUploadFirmwareTest extends Test {
	...
	public void start(){
		...
		((Activity)activityListener).runOnUiThread(new Runnable() {
			@Override
			public void run() {
				holder.donutProgress.setProgress(0);
			}
		});
...
//						initialised = firmWareUploader.deviceInit();
				...
				if(retries>=3){
					...
					((Activity)activityListener).runOnUiThread(new Runnable() {
						@Override
						public void run() {
							holder.donutProgress.setVisibility(View.INVISIBLE);
							holder.result.setVisibility(View.VISIBLE);
							holder.result.setIcon(GoogleMaterial.Icon.gmd_cancel);
							holder.result.setColor(Color.RED);
	...
	public void interrupt() {
		super.interrupt();
		try{dummyFirmWareUploader.stop();}catch(Exception e){};
